<x-app-layout>
  ...
  <div class="max-w-3xl mx-auto bg-white p-16 mt-6">
    <img src="{{ asset('/assets/images/fb474b70b4092f95c379e633ca58d27c.gif')}}" alt="">
  ...
  @endif
    <form class="mt-5" action="{{ route('dashboard.coursesStore')}}" method="POST" autocomplete="off" enctype="multipart/form-data">
      ...
      <div class="mb-6">
        ...
        <input type="text" id="title" name="title" value="{{ old('title')}}" placeholder="Kurs nomini yozing.." class="block w-full p-2.5 text-gray-900 border border-gray-300 rounded-lg bg-gray-50 sm:text-md focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required>
    </div> 
      <div class="grid gap-6 mb-6 lg:grid-cols-2">
          <div>
            <label for="narxi" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Kurs narxi</label>
            <input type="text" id="narxi" name="narxi" value="{{ old('narxi')}}" placeholder="Bepul, 120000 so'm" class="block w-full p-2.5 text-gray-900 border border-gray-300 rounded-lg bg-gray-50 sm:text-md focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required>
          </div>
          <div>
            ...
            <select id="kurs_tili" name="kurs_tili" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required>
                <option  selected value="uzbek_tilida" {{ old('kurs_tili') === 'uzbek_tilida' ? 'selected' : '' }}>O'zbek tilida</option>
                <option value="rus_tilida" {{ old('kurs_tili') === 'rus_tilida' ? 'selected' : '' }}>Rus tilida</option>
            </select>
        </div>
          <div>
              <label for="davomiylik_vaqti" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">Davomiylik vaqti</label>
              <input type="text" id="davomiylik_vaqti" value="{{ old('davomiylik_vaqti')}}" name="davomiylik_vaqti" placeholder="12 soat" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required>
          </div>  
          <div>
            <label for="category" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">Kurs bo'limini tanlang</label>
            <select id="category" name="category" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required>
                <option  selected value="logo" {{ old('category') === 'logo' ? 'selected' : '' }}>Logopediya</option>
                <option value="oligo" {{ old('category') === 'oligo' ? 'selected' : '' }}>Oligofrenopedagogika</option>
                <option value="surdo" {{ old('category') === 'surdo' ? 'selected' : '' }}>Surdopedagogika</option>
            </select>
          </div>
      ...
      <div class="flex items-start mb-6">
          ...
          <input id="status" type="checkbox" name="status" value="1"  {{ old('status') ? 'checked' : '' }} class="w-4 h-4 border border-gray-300 rounded bg-gray-50 focus:ring-3 focus:ring-blue-300 dark:bg-gray-700 dark:border-gray-600 dark:focus:ring-blue-600 dark:ring-offset-gray-800" >
          </div>
      ...
      <div class="flex justify-center">
        <button type="submit" class="text-white transition  bg-gradient-to-br from-purple-600 to-blue-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-xl px-5 py-2.5 text-center mx-auto mr-2 mb-8">Kurs yaratish</button>
      </div>
    ...
    <div class="flex p-4 mb-6 text-sm  rounded-lg " role="alert" style="background-color: #2d80fb; color: #fff">
      ...
      <div>
        ...
        <div class="mt-4">
          <button disabled type="button" class="cursor-not-allowed text-gray-900 bg-gray-100 hover:bg-gray-200 focus:ring-4 focus:outline-none focus:ring-gray-100 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:focus:ring-gray-500 mr-2 mb-2">
